...
public class TeleportController : MonoBehaviour
{
    ...
    private GameObject cur_planet;

    //testing
    public Renderer boxrender;
    private Color old;
    public Color newColor;
    public GameObject Havay;

    void Start()
    {
        old = boxrender.material.color; // record old color
    ...
    void LateUpdate()
    {
        updatepos(lastTeleportPoint);
        if (lastTeleportPoint.name == "TeleportPointHavay")
        {
            boxrender.material.color = newColor;
        ...
        {
            boxrender.material.color = old;
            //var child = Havay.transform.Find("Magma Burst");
            //if (child)
            //{

            //    child.GetComponent<ParticleSystem>().Stop(true);
            //}
        }
    }
    ...
    void Update()
    {
        ...
        {
            // positionIndicator.transform.position = lastTeleportPoint.GetDestTransform().position;
            cur_planet.SendMessage("AllowListen", false);
        }
        else
        {
            cur_planet.SendMessage("AllowListen", true);
    ...
    //
    void updatepos(TeleportPoint last)
    {
        if ((last != null))
        {
            transform.position = last.GetDestTransform().position;
            transform.rotation = last.GetDestTransform().rotation;
        }
    }
    ...
    IEnumerator TeleportCoroutine(Transform destTransform)
    {
        ...
        //start tracking new when faded to black
        updatepos(currentTeleportPoint);
        ...
        lastTeleportPoint = currentTeleportPoint;
